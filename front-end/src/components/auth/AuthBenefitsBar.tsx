...
const authBenefits = [
  [Truck, "Envío a todo El Salvador $4"],
...
export function AuthBenefitsBar() {
  return (
    <div className="bg-[#F992C0] text-white">
      <ul className="mx-auto grid w-full max-w-[1440px] grid-cols-2 gap-x-3 gap-y-3 px-4 py-3 sm:grid-cols-4 sm:gap-y-0 sm:px-6 lg:px-8">
        {authBenefits.map(([Icon, label]) => (
          <li
            key={label}
            className="flex min-w-0 items-center justify-center gap-2 text-center text-[10px] font-semibold text-white lg:gap-3 lg:text-xs"
          >
            <Icon
