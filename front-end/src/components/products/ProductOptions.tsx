...
export default function ProductOptions({variants}:{variants:ProductVariant[]}) {
  ...
  return <div className="mt-6 space-y-5">
    ...
    <div><p className="mb-3 text-sm font-semibold">Cantidad</p><div className="flex flex-wrap items-center gap-3"><div className="inline-flex h-11 items-center overflow-hidden rounded-md border border-[#d9dde5] bg-[#f2f5fb]"><button type="button" aria-label="Disminuir cantidad" disabled={quantity<=1} onClick={()=>setQuantity(v=>Math.max(1,v-1))} className="flex h-full w-10 items-center justify-center disabled:opacity-30"><Minus className="h-4 w-4"/></button><span className="min-w-8 text-center text-sm font-medium">{quantity}</span><button type="button" aria-label="Aumentar cantidad" disabled={!selected||quantity>=selected.stock} onClick={()=>setQuantity(v=>Math.min(selected?.stock??1,v+1))} className="flex h-full w-10 items-center justify-center disabled:opacity-30"><Plus className="h-4 w-4"/></button></div><button disabled={!available} className="flex h-11 min-w-44 items-center justify-center gap-2 rounded bg-[#1822d9] px-6 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300"><ShoppingCart className="h-4 w-4"/>Añadir al carrito</button></div>{selected&&<p className="mt-2 text-xs text-slate-500">Máximo {selected.stock} unidades</p>}</div>
