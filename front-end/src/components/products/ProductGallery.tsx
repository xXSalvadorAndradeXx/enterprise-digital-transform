...
export default function ProductGallery({ images, productName }: { images: ProductImage[]; productName: string }) {
  ...
  return <div>
    ...
    <div className="mt-4 flex gap-3 overflow-x-auto pb-2">{images.map((image,index)=><button key={image.id||image.url} onClick={()=>setSelected(index)} className={`relative h-20 w-20 shrink-0 overflow-hidden border-2 bg-[#f5f5f5] ${selected===index?"border-[#B80A18]":"border-transparent"}`} aria-label={`Ver imagen ${index+1}`}>{!failed.includes(image.url)&&<Image src={image.url} alt="" fill unoptimized className="object-contain p-1" onError={()=>setFailed(v=>[...v,image.url])}/>}</button>)}</div>
  </div>;
}
