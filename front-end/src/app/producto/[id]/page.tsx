...
export default async function ProductDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="mx-auto w-full max-w-[1280px] px-5 py-6 sm:px-8">
    <nav aria-label="Ruta de navegación" className="flex items-center gap-2 bg-[#FDE3EE] px-5 py-4 text-xs text-[#7A2631]">
      <Link href="/" className="transition-colors hover:text-[#B80A18] hover:underline">Inicio</Link>
      <span aria-hidden="true">›</span>
      <Link href="/productos" className="transition-colors hover:text-[#B80A18] hover:underline">Producto</Link>
      <span aria-hidden="true">›</span>
    ...
    <header className="border-b border-[#aeb4bd] px-3 pb-8 pt-10 sm:px-4">
      ...
      <p className="mt-8 text-sm font-bold text-[#B80A18]">Detalles del producto</p>
    </header>
