...
export default function ComprasListView({ showEmptyState = false }: ComprasListViewProps) {
  ...
  const columns = useMemo<readonly TableColumn<PurchaseResponse>[]>(() => [
    ...
    { id: "invoice", header: "Factura", className: "w-[12%]", align: "center", render: (row) => row.invoiceUrl ? <InvoiceThumbnail src={row.invoiceUrl} alt={`Factura ${row.reference}`} fileType={row.invoiceUrl.toLowerCase().includes(".pdf") ? "pdf" : "image"} className="mx-auto" /> : <span className="text-gray-500">Sin factura</span> },
