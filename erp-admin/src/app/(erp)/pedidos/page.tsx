    ...
    <main className="space-y-8 pb-10 text-[#4A4A4A]">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-300 bg-white">
        ...
        {error ? <div className="grid min-h-72 place-items-center p-8 text-center"><div><p className="text-lg font-semibold text-red-600">{error}</p><button onClick={load} className="mt-4 rounded-lg bg-[#2424DF] px-6 py-3 font-semibold text-white">Reintentar</button></div></div>
          ...
          : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="border-b border-slate-300 bg-slate-50 text-left"><tr>{["ID", "Cliente", "Fecha", "Total", "Entrega", "Estado"].map((heading) => <th key={heading} className="px-5 py-4 font-medium">{heading}</th>)}</tr></thead><tbody>{result.items.map((order: AdminOrderListItem) => <tr key={order.id} className="border-b border-slate-200 hover:bg-blue-50/50"><td className="px-5 py-4"><Link className="font-semibold text-[#1717B8] hover:underline" href={`/pedidos/${order.orderNumber}`}>#{order.orderNumber}</Link></td><td className="px-5 py-4"><p className="font-medium">{order.customerName}</p><p className="mt-1 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-slate-500">{order.customerEmail && <><span className="break-all">{order.customerEmail}</span><span aria-hidden="true">·</span></>}<span>{order.customerType === "REGISTERED" ? "Registrado" : "Invitado"}</span></p></td><td className="px-5 py-4">{formatDate(order.createdAt)}</td><td className="px-5 py-4 font-medium">{formatMoney(order.total)}</td><td className="px-5 py-4"><span className="flex items-center gap-2"><DeliveryIcon type={order.deliveryType} />{deliveryLabels[order.deliveryType] ?? order.deliveryType}</span></td><td className="px-5 py-4"><span style={statusColors[order.status]} className="inline-flex whitespace-nowrap rounded-full px-3 py-1 text-xs font-medium">{statusLabels[order.status]}</span></td></tr>)}</tbody></table></div>}
