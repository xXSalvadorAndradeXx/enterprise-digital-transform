...
export class SuppliersController {
  ...
  @ApiOperation({
    ...
    description:
      'Devuelve una lista paginada de proveedores ordenados por fecha de creación descensional. Permite filtrar por nombre o contacto de manera case-insensitive.',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiQuery({
    ...
    type: String,
    description:
      'Término de búsqueda case-insensitive sobre el nombre o persona de contacto',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna los datos de un proveedor específico identificado por su UUID v4. Retorna 404 si no existe o fue eliminado por soft delete.',
  })
  @ApiParam({
    name: 'id',
    description: 'UUID v4 del proveedor',
    type: String,
  ...
  @ApiOperation({
    ...
    description:
      'Registra un nuevo proveedor en el sistema. Valida la unicidad del nombre de forma case-insensitive y normaliza el número telefónico para El Salvador (+503).',
  })
  @ApiBody({
  ...
  @ApiResponse({
    status: 409,
    description:
      'Conflicto: Ya existe un proveedor registrado con ese mismo nombre (case-insensitive)',
  })
  @ApiResponse({
    status: 422,
    description:
      'Error de validación en los campos del cuerpo DTO (ej. nombre corto, email o teléfono inválido)',
    schema: {
      example: {
        statusCode: 422,
        message: [
  ...
  @ApiOperation({
    ...
    description:
      'Modifica uno o varios campos de un proveedor existente. Si se actualiza el nombre, se valida la unicidad. Si se modifica el teléfono, se aplica la normalización (+503).',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiBody({
    ...
    description: 'Campos del proveedor a modificar parcialmente',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 409,
    description:
      'Conflicto: Ya existe otro proveedor con el nombre especificado',
  })
  @ApiResponse({
    status: 422,
  ...
  @ApiOperation({
    ...
    description:
      'Aplica eliminación lógica marcando la fecha en "deleted_at". Bloquea la eliminación respondiendo 409 si el proveedor tiene compras en estado PENDIENTE o RECIBIDA.',
  })
  @ApiParam({
    name: 'id',
    description: 'UUID v4 del proveedor a eliminar',
    type: String,
    example: '123e4567-e89b-12d3-a456-426614174000',
  })
  @ApiResponse({
    status: 204,
    description:
      'Proveedor eliminado exitosamente (sin contenido en la respuesta)',
  })
  @ApiResponse({
    status: 400,
    description: 'Formato de UUID inválido',
  ...
  @ApiResponse({
    status: 409,
    description:
      'Conflicto: No se puede eliminar el proveedor porque tiene compras activas en estado Pendiente o Recibida',
  })
