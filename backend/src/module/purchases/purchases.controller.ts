...
export class PurchasesController {
  ...
  @ApiOperation({
    summary:
      'Subir factura (PNG, JPG, PDF — máx 10 MB). Llamar antes de confirmar compra.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Crea supplier_purchase + items + inventories + inventory_details + inventory_movements.',
  })
  @ApiBody({ type: CreateNewProductPurchaseDto })
  @ApiResponse({ status: 201, type: PurchaseResponseDto })
  @ApiResponse({
    status: 404,
    description: 'Proveedor o categoría no encontrada',
  })
  @ApiResponse({ status: 409, description: 'Doble envío detectado (RN-029)' })
  @ApiResponse({
    status: 422,
    description: 'Variantes duplicadas u otras validaciones',
  })
  @ApiResponse({
    status: 500,
    description: 'Fallo en transacción — rollback automático',
  })
  createNuevoProducto(
  ...
  @ApiOperation({
    ...
    description:
      'Crea supplier_purchase + items, actualiza stock en inventory_details, inserta inventory_movements.',
  })
  @ApiBody({ type: CreateRestockPurchaseDto })
  @ApiResponse({ status: 201, type: PurchaseResponseDto })
  @ApiResponse({
    status: 404,
    description: 'Inventario o detalle no encontrado',
  })
  @ApiResponse({
    status: 409,
    description: 'Detalle no pertenece al inventario / doble envío',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '{ data: PurchaseResponseDto[], meta }',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '{ statusCode, data: RestockInventoryOption[] }',
  })
  ...
  @ApiOperation({
    summary:
      'Vista previa de inventario para pre-poblar el formulario de reabastecimiento',
  })
  ...
  @ApiOperation({
    summary:
      'Historial de compras de un inventario (NUEVO_PRODUCTO + REABASTECIMIENTO)',
  })
  @ApiResponse({
    status: 200,
    description: '{ data: PurchaseResponseDto[], meta }',
  ...
  @ApiResponse({
    status: 404,
    description: 'Compra, proveedor, categoría o variante no encontrada',
  })
  @ApiResponse({
    status: 409,
    description: 'Combinación talla-color duplicada',
  })
  @ApiResponse({
    status: 422,
    description: 'Stock quedaría negativo en alguna variante',
  })
  @ApiResponse({
    status: 500,
    description: 'Fallo en transacción — rollback automático',
