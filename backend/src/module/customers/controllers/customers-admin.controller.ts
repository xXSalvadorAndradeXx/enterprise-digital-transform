...
import { CustomerOrdersService } from '../../orders/services/customer-orders.service';
...
export class CustomersAdminController {
  constructor(
    ...
    @Inject(forwardRef(() => CustomerOrdersService))
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene la lista de clientes registrados en la plataforma con soporte para búsqueda, filtrado por estado y paginación. Requiere el permiso "customers:read".',
  })
  @ApiOkResponse({
  ...
  @ApiForbiddenResponse({
    description:
      'Acceso denegado: El usuario no cuenta con el permiso "customers:read" requerido.',
  })
  @Get()
  async findAll(@Query() query: FindCustomersQueryDto) {
    ...
    const { customers, meta } = await this.customersService.findAllForAdmin(
      page,
      limit,
      filters,
    );
    ...
    const formattedCustomers = customers.map((c) => {
      ...
      return plainToInstance(CustomerAdminResponseDto, mapped, {
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene la información detallada de un cliente registrado por su ID. Requiere el permiso "customers:read".',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiNotFoundResponse({
    description:
      'No encontrado: No se encontró el cliente con el ID especificado.',
  })
  @ApiUnauthorizedResponse({
    description: 'No autorizado: Token de acceso no válido o no enviado.',
  })
  ...
  async findOne(
    ...
    const serialized = plainToInstance(CustomerAdminResponseDto, mapped, {
      excludeExtraneousValues: true,
    });
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene los pedidos asociados a un cliente con paginación. Requiere el permiso "customers:read".',
  })
  @ApiParam({
    name: 'id',
    description: 'ID del cliente (UUID v4)',
  ...
  async findOrders(
    ...
    const limit = query.limit || 10;
    const { orders, meta } =
      await this.customerOrdersService.findOrdersForCustomer(id, page, limit);
